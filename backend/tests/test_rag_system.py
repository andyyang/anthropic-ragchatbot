...
class TestRAGSystemContentQueries:
    ...
    @patch("rag_system.DocumentProcessor")
    @patch("rag_system.SessionManager")
    def test_content_query_with_tool_use(
        self, mock_session, mock_doc_proc, mock_ai_gen, mock_vector_store, test_config
    ):
        ...
        mock_ai_gen.return_value.generate_response.return_value = (
            "MCP is a protocol for AI integration based on the course content."
        )
    ...
    @patch("rag_system.DocumentProcessor")
    @patch("rag_system.SessionManager")
    def test_course_specific_content_query(
        self, mock_session, mock_doc_proc, mock_ai_gen, mock_vector_store, test_config
    ):
        ...
        rag.tool_manager.get_last_sources = Mock(
            return_value=[
                {
                    "text": "Advanced Python Programming - Lesson 1",
...
Lesson 2: Basic Concepts"""
    ...
    @patch("rag_system.DocumentProcessor")
    @patch("rag_system.SessionManager")
    def test_general_knowledge_query_no_tools(
        self, mock_session, mock_doc_proc, mock_ai_gen, mock_vector_store, test_config
    ):
    ...
    def test_add_course_folder_success(
        ...
        courses_and_chunks = [
            (
                ...
                [
                    CourseChunk(
                        content="Content 1", course_title="Course 1", chunk_index=0
                    )
                ],
            ),
            (
                ...
                [
                    CourseChunk(
                        content="Content 2", course_title="Course 2", chunk_index=0
                    )
                ],
            ),
            ...
            (
                ...
                [
                    CourseChunk(
                        content="Content 3", course_title="Course 3", chunk_index=0
                    )
                ],
            ),
        ]
...
class TestRAGSystemErrorHandling:
    ...
    def test_ai_generator_error_handling(
        ...
        mock_ai_gen.return_value.generate_response.side_effect = Exception(
            "AI API Error"
        )
        mock_session.return_value.get_conversation_history.return_value = None
    ...
    @patch("rag_system.DocumentProcessor")
    @patch("rag_system.SessionManager")
    def test_tool_manager_error_graceful_handling(
        self, mock_session, mock_doc_proc, mock_ai_gen, mock_vector_store, test_config
    ):
        ...
        # Mock AI response
        mock_ai_gen.return_value.generate_response.return_value = (
            "Fallback response due to tool error"
        )
